...
| The report says what wrote it | `/report` and `VALIDATION-REPORT.md` open with one sentence an assessor can read without knowing the tool — "No language model produced or judged any part of this report", or which parts one did — and every evidence row carries a `Written by` column (`TABLE` / `TOOL` / `MODEL`). The report never names a model that did not run |
...
no API key, every one `reviewed`, and traces each safety goal down to its test cases. With cppcheck and
semgrep installed (`brew install cppcheck semgrep`), a second pass leaves **one** release blocker: the
S×E×C table, which you fill in `_reference-register/asil-table.yaml` from your licensed standard, or in
the dashboard under **Ready to run**. Analyser output lands in `input/reports/` and is gitignored — it
is regenerated by every run and is never a source of truth.
